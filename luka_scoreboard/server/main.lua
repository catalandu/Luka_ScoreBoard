...
ESX.RegisterServerCallback('luka_scoreboard:getConnectedPlayers', function(source, cb)
	cb(connectedPlayers)
end)
...
AddEventHandler('esx:setJob', function(playerId, job, lastJob)
	...
	TriggerClientEvent('luka_scoreboard:updateConnectedPlayers', -1, connectedPlayers)
end)
...
function AddPlayerToScoreboard(xPlayer, update)
	...
	if xPlayer.player.getGroup() == 'user' then
		Citizen.CreateThread(function()
			Citizen.Wait(3000)
			TriggerClientEvent('luka_scoreboard:toggleID', playerId, false)
		end)
	end
end
...
function UpdatePing()
	...
	TriggerClientEvent('luka_scoreboard:updatePing', -1, connectedPlayers)
end
...
TriggerEvent('es:addGroupCommand', 'sctoggle', 'admin', function(source, args, user)
	TriggerClientEvent('luka_scoreboard:toggleID', source)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SISTEMA', 'Non hai i permessi per farlo.' } })
